...
def export_tableau_ready(df: pd.DataFrame, output_path: Path) -> None:
    """
    Export a curated dataset for Tableau.

    This keeps modeling/feature logic in Python while Tableau focuses on storytelling.
    """
    keep_cols = [
        "Country",
        "Year",
    ...
    tableau_df = df[[c for c in keep_cols if c in df.columns]].copy()

    # Map ordered cluster labels (1=lowest life expectancy) to readable names
    cluster_name_map = {
        1: "High Risk",
        2: "Elevated Risk",
        3: "Moderate Risk",
    ...
    if "cluster_label" in tableau_df.columns:
        tableau_df["cluster_name"] = tableau_df["cluster_label"].map(cluster_name_map)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    tableau_df.to_csv(output_path, index=False)
    print(f"Tableau-ready dataset saved to: {output_path}")
...
def main() -> None:
    ...
    # Export Tableau-ready dataset for dashboarding
    tableau_path = BASE_DIR / "reports" / "tableau_ready.csv"
    export_tableau_ready(df_clusters, tableau_path)
